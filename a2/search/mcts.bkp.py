...
class MCTS(HexSearchMethod):
    ...
    def get_next_move(self, board, color):
        ...
        for _ in range(self.iterations):
            ...
            selected_child_board = self.expand(selected_leaf, HexBoard.from_hash_code(selected_leaf), color) # Expand

            if selected_child_board is not None:
                reward = self.simulate(selected_child_board, color, board.get_opposite_color(color)) # Simulate

                selected_path.append(selected_child_board.hash_code())
    ...
    def select(self, board):
        ...
        while True:
            path.append(node)
            if self.debug: self.log(2, 'Select - add to path', HexBoard.from_hash_code(node))
    ...
    def expand(self, selected_leaf, selected_board, color):
        if HexBoard.from_hash_code(selected_leaf).game_over():
            if self.debug: self.log(1, 'Expand - terminal')
            self.children[selected_leaf] = []
            self.visits[selected_leaf] = 1
            return None
        ...
        return HexBoard.from_hash_code(random.choice(new_children)) if len(new_children) > 0 else None
    ...
    def backpropagate(self, path, reward):
        ...
        for node_hc in reversed(path):
            ...
            reward = 1 - reward
            if self.debug: self.log(2, 'Backpropagate - reward %d' % (self.rewards[node_hc]), HexBoard.from_hash_code(node_hc))
